...
(function() {
  var Getopt, ParsedOption, fs, __matches,
  ...
  Getopt = (function() {
    ...
    Getopt.prototype.parse = function(argv) {
      var arg, content, e, i, long_name, option, rsp_args, rt_argv, rt_options, short_name, short_names, value, _i, _len;
      try {
        argv = argv.slice(0);
        rt_options = {};
        rt_argv = [];
        while ((arg = argv.shift()) != null) {
          if (__matches = arg.match(/^@.+/)) {
            content = fs.readFileSync(arg.substring(1), "utf8");
            rsp_args = content.split("\n");
            rsp_args = rsp_args.map(function(arg) {
              return arg.trim();
            });
            Array.prototype.unshift.apply(argv, rsp_args);
